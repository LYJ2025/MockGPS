...
到本仓库的 **Releases** 页面下载最新版 `app-debug-vX.X.apk`（当前为 **v1.32**），传到手机后用文件管理器点击安装即可。
...
| **v1.32** | 修复「轨迹模拟/暂停时切到主页再切回轨迹，蓝点 / 绿色起点 / 绿线不显示」：根因是 osmdroid 6.x 默认在 `MapView.onDetachedFromWindow` 时自动调用 `onDetach()`，`DefaultOverlayManager.onDetach()` 会清空整个 overlays 列表；ViewPager2 切页触发该流程后，切回时 Polyline 内部 `LinearRing` 被清空导致 `setPoints()` 抛 NPE，Marker 对象虽持有引用但已不在 overlays 列表中。修复：在 `BaseMapFragment.setupMap` 中调用 `mapView.setDestroyMode(false)` 关闭自动 detach，改由 `onDestroyView` 手动调用 `mapView.onDetach()`，这样切页时 overlays 列表不会被清空。版本号 1.31 → 1.32。 |
